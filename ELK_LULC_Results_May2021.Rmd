...
tiff('/Users/Mushy 1/Google Drive (wenjing.xu@berkeley.edu)/RESEARCH/Elk/GYE_ELK_LandUse/result/GYE_graph_cmpts/ownership_grey.tiff', units="in", width=11, height=6, res=300, compression = 'lzw')
data1 %>%
...
tiff('/Users/Mushy 1/Google Drive (wenjing.xu@berkeley.edu)/RESEARCH/Elk/GYE_ELK_LandUse/result/GYE_graph_cmpts/protection_grey.tiff', units="in", width=11, height=6, res=300, compression = 'lzw')
data22 %>%
  ggplot(aes(x=Herd, y=Value, fill = Category)) +
  geom_bar(position = "stack", stat = "identity") +  
  #scale_fill_manual(values=wes_palette("Chevalier1")) +
...
tiff('/Users/Mushy 1/Google Drive (wenjing.xu@berkeley.edu)/RESEARCH/Elk/GYE_ELK_LandUse/result/GYE_graph_cmpts/supp-protection_level_grey.tiff', units="in", width=11, height=6, res=300, compression = 'lzw')
data23 %>%
  ggplot(aes(x=Herd, y=Value, fill = Category)) +
  geom_bar(position = "stack", stat = "identity") +  
...
sp6 %>%
  ...
  geom_sf(aes(fill = Value), size = 0.01) +
  coord_sf(datum = NA) +
  scale_fill_gradient(name = "fence density /km2 ", low = "#ededed", high= "#3d3d3d", na.value = NA) +
  theme_ipsum(base_size = 12, axis_text_size = 5) +
  theme(legend.position="bottom", legend.direction = "horizontal", legend.key.size = unit(0.3, 'cm'), legend.title = element_text(size = 10), legend.text = element_text(size = 8)) 
dev.off()
summary(sp6$Value)
